...
- **수치는 어느 쪽에서 열어도 폰에서 온다.** 폰이 꺼져 있으면 "응답 없음"으로 표시된다.
...
주소에 쿼리 문자열을 붙여 캐시를 피하자 새 사이트가 나왔다. 평소에는 `gh-pages`에 push하면 빌드가 시작된다. 첫 전환 때만 겪는 일이다.
...
- **요청 한도는 여전히 빠듯하다.** 대시보드 탭 하나를 5초 간격으로 열어 두면 시간당 720건이다. 월 20,000건은 그런 탭 하나가 약 28시간 버티는 양이다. 방문자가 늘면 이 한도가 먼저 바닥난다. 한도를 넘었을 때 ngrok이 어떻게 동작하는지는 아직 겪어 보지 않았다.
